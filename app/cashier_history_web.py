...
def history(bar_id: int):
    ...
    if entry_type in {"ALL", "PAYMENT"}:
        payment_rows = list(
            db.session.execute(
                select(Payment, Order.reference, Order.assigned_staff_id)
                .join(Order, (Order.id == Payment.order_id) & (Order.bar_id == Payment.bar_id))
                .where(*payment_filters)
    ...
    if entry_type in {"ALL", "REFUND"}:
        refund_rows = list(
            db.session.execute(
                select(Refund, Order.reference, Order.assigned_staff_id)
                .join(Order, (Order.id == Refund.order_id) & (Order.bar_id == Refund.bar_id))
                .where(*refund_filters)
    ...
    events = []
    for payment, order_reference, assigned_staff_id in payment_rows:
        events.append(
            {
                "kind": "PAYMENT",
    ...
    for refund, order_reference, assigned_staff_id in refund_rows:
        events.append(
            {
                "kind": "REFUND",
