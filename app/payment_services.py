...
class PaymentService:
    ...
    def record(self, actor, bar_id, order_id, reference, method, presented, applied, change=0, cash_session_id=None, staff_assignment_id=None, provider_code=None, provider_transaction_id=None):
        ...
        if applied<=0 or change<0 or presented!=applied+change: raise ValueError("INVALID_PAYMENT_AMOUNTS")
        if method not in {"CASH","CARD","MOBILE_MONEY","BANK_TRANSFER"}: raise ValueError("INVALID_METHOD")
        if applied>order_balance(order)["amount_due"]: raise ValueError("PAYMENT_LIMIT_EXCEEDED")
        if (provider_code is None)!=(provider_transaction_id is None): raise ValueError("PROVIDER_REFERENCE_REQUIRED")
        if provider_code is not None:
            provider_code=required_text(provider_code,32);provider_transaction_id=required_text(provider_transaction_id,128)
        if method=="CASH":
            if provider_code is not None: raise ValueError("INVALID_CASH_PROVIDER")
            if (cash_session_id is None)==(staff_assignment_id is None): raise ValueError("CASH_LOCATION_REQUIRED")
        ...
        item=Payment(bar_id=bar_id,order_id=order_id,reference=required_text(reference,64),amount=applied,amount_presented=presented,amount_applied=applied,change_given=change,currency=order.currency,method=method,cash_session_id=cash_session_id,staff_assignment_id=staff_assignment_id,cash_holder=("STAFF" if staff_assignment_id is not None else "DRAWER") if method=="CASH" else None,provider_code=provider_code,provider_transaction_id=provider_transaction_id,received_at=utcnow(),recorded_by_id=actor.id)
        db.session.add(item);db.session.flush()
        if method=="CASH":
            cash_service.entry(actor,bar_id,applied,order.currency,f"Paiement {reference}",session_id=cash_session_id,staff_id=staff_assignment_id,payment_id=item.id)
    ...
    def refund(self, actor, bar_id, payment_id, reference, amount, reason, order_return_id=None, cash_session_id=None, staff_assignment_id=None):
        ...
        if order.status not in {"CONFIRMED","SERVED"}: raise ValueError("ORDER_NOT_REFUNDABLE")
        refunded=total(Refund,Refund.amount,bar_id=bar_id,payment_id=payment_id)
        if refunded+amount>payment.amount_applied: raise ValueError("REFUND_PAYMENT_LIMIT")
        if order_return_id is not None:
            returned=db.session.scalar(select(OrderReturn).where(OrderReturn.id==order_return_id,OrderReturn.bar_id==bar_id,OrderReturn.order_id==order.id,OrderReturn.status=="POSTED"))
            if not returned: raise LookupError("NOT_FOUND")
            previous=total(Refund,Refund.amount,bar_id=bar_id,order_return_id=order_return_id)
            if previous+amount>returned.total_amount: raise ValueError("REFUND_RETURN_LIMIT")
            if amount>order_balance(order)["refundable_overpayment"]: raise ValueError("REFUND_OVERPAYMENT_LIMIT")
        if payment.method=="CASH":
            if (cash_session_id is None)==(staff_assignment_id is None): raise ValueError("CASH_LOCATION_REQUIRED")
        elif cash_session_id is not None or staff_assignment_id is not None: raise ValueError("INVALID_NONCASH_REFUND")
        item=Refund(bar_id=bar_id,payment_id=payment.id,order_id=order.id,order_return_id=order_return_id,reference=reference,amount=amount,currency=payment.currency,method=payment.method,cash_session_id=cash_session_id,staff_assignment_id=staff_assignment_id,cash_holder=("STAFF" if staff_assignment_id is not None else "DRAWER") if payment.method=="CASH" else None,reason=reason,refunded_at=utcnow(),recorded_by_id=actor.id)
        db.session.add(item);db.session.flush()
        if payment.method=="CASH":
            cash_service.entry(actor,bar_id,-amount,payment.currency,reason,session_id=cash_session_id,staff_id=staff_assignment_id,refund_id=item.id)
        order_balance(order,update=True)
    ...
    def cancel_paid(self, actor, bar_id, order_id, reference, reason, cash_session_id=None, staff_assignment_id=None):
        ...
        for payment in db.session.scalars(select(Payment).where(Payment.bar_id==bar_id,Payment.order_id==order_id).order_by(Payment.id)):
            remaining=payment.amount_applied-total(Refund,Refund.amount,bar_id=bar_id,payment_id=payment.id)
            if remaining:
                self.refund(actor,bar_id,payment.id,f"{reference}-{payment.id}",remaining,reason,cash_session_id=cash_session_id if payment.method=="CASH" else None,staff_assignment_id=staff_assignment_id if payment.method=="CASH" else None)
