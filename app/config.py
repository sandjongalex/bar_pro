...
def validate_config(config: dict) -> None:
    ...
    if config.get("ENVIRONMENT") == "production":
        ...
        if scheme != "mysql+pymysql":
            raise ConfigError(
                "Production requires a mysql+pymysql SQLALCHEMY_DATABASE_URI."
            )
        ...
        try:
            ...
            if not isinstance(private.curve, SECP256R1):
                raise ValueError()
        ...
        if (
            ...
            raise ConfigError(
                ...
                "Set ALLOW_IN_MEMORY_RATELIMIT=1 only for "
                "constrained single-worker deployments."
            )


def load_environment(config) -> None:
    """Read environment variables and JWT key files."""
    ...
    for name in environment_variables:
        if (
            ...
            and not (
                config.get("TESTING")
                and name == "SQLALCHEMY_DATABASE_URI"
            )
        ):
    ...
    for name in (
        ...
        if path:
            try:
                config[name] = Path(path).read_text(
                    encoding="utf-8"
                )
            except OSError:
                raise ConfigError(
